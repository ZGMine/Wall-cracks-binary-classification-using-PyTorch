...
Le modèle atteint une **précision globale de 80%** sur le jeu de données de test.

*   **Précision sur les fissures (Crack) :** ~86% (Le transfert de connaissances a très bien fonctionné pour cette classe).
*   **Précision sur les non-fissures (Non-Crack) :** ~26% (Le modèle peine à généraliser sur les textures de murs sains qu'il n'a pas vu lors de l'entraînement).
