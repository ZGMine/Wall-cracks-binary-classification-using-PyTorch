...
*   `/code` : Le notebook/script Python contenant l'implémentation du modèle, l'entraînement et l'évaluation.
*   `/data` : Le dataset utilisé pour le fine-tuning (fissures de sol).
*   `/report` : Le rapport final au format PDF (`Rapport_Final_Blanco.pdf`) détaillant la méthodologie et l'analyse théorique.
